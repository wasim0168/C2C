...
const { sendOTP, verifyOTP } = require('./utils/emailService.js');
// Initialize Razorpay
...
// 2FA Rate Limiter (prevent brute force)
...
app.get('/', async (req, res) => {
  try {
    ...
    const limit = 10; // 9 products per page
...
app.post('/plans/create-order', isAuthenticated, async (req, res) => {
  try {
    ...
    const order = await razorpay.orders.create({
      ...
      currency: 'INR',
      receipt: `order_${Date.now()}_${req.session.user.id}`,
      notes: {
...
// Add this middleware to verify payment before plan upgrade
const verifyPlanUpgrade = async (req, res, next) => {
  try {
    const { plan_id } = req.body;
    const userId = req.session.user.id;

    // Check if user already has this plan
    const [user] = await db.query('SELECT plan_id FROM users WHERE id = ?', [userId]);
    if (user[0].plan_id === parseInt(plan_id)) {
      return res.status(400).json({ error: 'You already have this plan' });
    }

    // Check if payment exists for this upgrade
    ...
    console.error('Plan upgrade verification error:', error);
    res.status(500).json({ error: 'Plan upgrade verification failed' });
  }
};

// Apply this middleware to your plan upgrade route
app.post('/plans/upgrade', isAuthenticated, verifyPlanUpgrade, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      await db.query(
        ...
        (user_id, plan_id, amount, razorpay_payment_id, razorpay_order_id, razorpay_signature, status) 
        VALUES (?, ?, ?, ?, ?, ?, 'completed')`,
        [userId, plan_id, plan[0].price, razorpay_payment_id, razorpay_order_id, razorpay_signature]
      );

      // 5. Update user's plan
      await db.query(
        'UPDATE users SET plan_id = ?, product_limit = ? WHERE id = ?',
        [plan_id, plan[0].max_products, userId]
      );
    ...
    return res.status(500).json({ 
      success: false, 
      message: error.sqlMessage || 'Internal server error during payment verification' 
    });
  }
});
// Verify Payment
app.post('/plans/verify-payment', isAuthenticated, async (req, res) => {
  try {
    ...
    // 4. Verify payment with Razorpay API
    ...
    try {
      ...
      // 8. Record payment
      await connection.query(
        'INSERT INTO payments (user_id, plan_id, amount, razorpay_payment_id, razorpay_order_id, status) VALUES (?, ?, ?, ?, ?, ?)',
        [req.session.user.id, plan_id, plan[0].price, razorpay_payment_id, razorpay_order_id, 'completed']
      );
...
app.post('/admin/login', [
  ...
  body('csrf_token').notEmpty() // CSRF token validation
], async (req, res) => {
  // CSRF Validation
  ...
  try {
    ...
    // 2FA Verification (if enabled for admin)
    if (admin.totp_secret) {
      if (!totp_token) {
        return res.render('admin/login-2fa', { // Separate 2FA page
          email,
          
        });
      }

      // Apply rate limiting only to 2FA attempts
      twoFALimiter(req, res, async () => {
        const verified = speakeasy.totp.verify({
          secret: admin.totp_secret,
          encoding: 'base32',
          token: totp_token,
          window: 1 // Allow 1 token before/after current
        });

        if (!verified) {
          securityLogStream.write(`[2FA FAIL] Admin ${email} from ${req.ip}\n`);
          return res.render('admin/login', {
    ...
    req.session.regenerate(() => {
      ...
      securityLogStream.write(`[ADMIN LOGIN] ${email} from ${req.ip}\n`);
      res.redirect('/admin/dashboard');
    });

  } catch (error) {
    securityLogStream.write(`[ADMIN LOGIN ERROR] ${error.stack}\n`);
    res.render('admin/login', {
...
app.post('/auth/login', [
  ...
  if (failedAttempts.has(ip) && failedAttempts.get(ip).count >= MAX_FAILED_ATTEMPTS) {
    ...
    if (now - lockoutTime < LOCKOUT_TIME) {
      ...
      securityLogStream.write(`[SECURITY] IP ${ip} locked out - attempted login during lockout period\n`);
      return res.render('auth/login', { 
  ...
  if (!errors.isEmpty()) {
    securityLogStream.write(`[VALIDATION FAILED] Login attempt with invalid email format from IP: ${ip}\n`);
    return res.render('auth/login', { 
  ...
  try {
    // Log login attempt
    securityLogStream.write(`[LOGIN ATTEMPT] Email: ${email} from IP: ${ip}\n`);

    const [users] = await db.query('SELECT * FROM users WHERE email = ?', [email]);
    
    if (users.length === 0) {
      recordFailedAttempt(ip);
      securityLogStream.write(`[FAILED LOGIN] Invalid email: ${email} from IP: ${ip}\n`);
      return res.render('auth/login', { 
        error: 'Invalid email or password',
        remainingAttempts: MAX_FAILED_ATTEMPTS - (failedAttempts.get(ip)?.count || 0)
      });
    }
    ...
    if (user.is_blocked) {
      securityLogStream.write(`[BLOCKED ACCOUNT] Attempt to access blocked account: ${email}\n`);
      return res.render('auth/login', { 
        error: 'Your account has been blocked. Please contact support.'
      });
    }
    ...
    if (!match) {
      recordFailedAttempt(ip);
      const remaining = MAX_FAILED_ATTEMPTS - failedAttempts.get(ip).count;
      securityLogStream.write(`[FAILED LOGIN] Invalid password for: ${email} from IP: ${ip}\n`);
    ...
    req.session.regenerate((err) => {
      if (err) {
        securityLogStream.write(`[SESSION ERROR] ${err.message}\n`);
      ...
      securityLogStream.write(`[SUCCESSFUL LOGIN] User: ${user.email} from IP: ${ip}\n`);
    ...
    securityLogStream.write(`[LOGIN ERROR] ${error.stack || error.message}\n`);
...
/**
 * Records failed login attempts and implements account lockout
 * @param {string} ip - IP address of the client
 * @param {string} [email] - Optional email for user-specific tracking
 */
async function recordFailedAttempt(ip, email = 'unknown') {
  const now = Date.now();
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production' && redisClient) {
      // Increment attempt count with Redis
      const attempts = await redisClient.incr(attemptKey);
      
      // Set expiration if this is the first attempt
      if (attempts === 1) {
        await redisClient.expire(attemptKey, LOCKOUT_TIME / 1000);
      }

      // Log every failed attempt
      securityLogStream.write(`[FAILED ATTEMPT] IP: ${ip} | Email: ${email} | Attempt: ${attempts}\n`);

      // If reached max attempts, set lockout
      if (attempts >= MAX_FAILED_ATTEMPTS) {
        await redisClient.setex(lockoutKey, LOCKOUT_TIME / 1000, 'locked');
        securityLogStream.write(`[ACCOUNT LOCKED] IP: ${ip} | Email: ${email}\n`);
        
        // Optionally send security alert
        if (process.env.SECURITY_ALERT_WEBHOOK) {
          sendSecurityAlert(ip, email, attempts);
        }
      }
    } 
    // In development, use in-memory tracking
    else {
      if (!failedAttempts.has(ip)) {
        ...
        // Log every failed attempt after the first one
        if (attempt.count > 1) {
          securityLogStream.write(
            `[FAILED ATTEMPT] IP: ${ip} | ` +
            `Emails: ${Array.from(attempt.emails).join(', ')} | ` +
      ...
      if (failedAttempts.get(ip)?.count >= MAX_FAILED_ATTEMPTS) {
        securityLogStream.write(
          `[ACCOUNT LOCKED] IP: ${ip} | ` +
          `Emails: ${Array.from(failedAttempts.get(ip).emails).join(', ')}\n`
        );
      }
    }
  } catch (error) {
    console.error('Failed attempt recording error:', error);
    securityLogStream.write(`[ERROR] Failed to record attempt: ${error.message}\n`);
  }
}

// Helper function to send security alerts (example implementation)
async function sendSecurityAlert(ip, email, attempts) {
  try {
    const response = await fetch(process.env.SECURITY_ALERT_WEBHOOK, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        text: `⚠️ Security Alert: Multiple failed login attempts\n` +
              `• IP: ${ip}\n` +
              `• Email: ${email || 'unknown'}\n` +
              `• Attempts: ${attempts}\n` +
    ...
    securityLogStream.write(`[ALERT ERROR] Failed to send security alert: ${error.message}\n`);
  }
}

// Helper function to check if an IP is currently locked out
async function isIpLockedOut(ip) {
  try {
    if (process.env.NODE_ENV === 'production' && redisClient) {
      const locked = await redisClient.exists(`lockout:${ip}`);
      return locked === 1;
    }
    return failedAttempts.has(ip) && 
           failedAttempts.get(ip).count >= MAX_FAILED_ATTEMPTS &&
           Date.now() - failedAttempts.get(ip).time < LOCKOUT_TIME;
  } catch (error) {
    securityLogStream.write(`[LOCKOUT CHECK ERROR] ${error.message}\n`);
...
async function getRemainingAttempts(ip) {
  try {
    ...
    return MAX_FAILED_ATTEMPTS - (failedAttempts.get(ip)?.count || 0);
  } catch (error) {
    securityLogStream.write(`[ATTEMPT COUNT ERROR] ${error.message}\n`);
    return MAX_FAILED_ATTEMPTS; // Return max to avoid revealing system state
  }
}
...
app.post('/auth/resend-otp', [
  ...
  try {
    ...
    res.json({ 
      success: sent,
      message: sent ? 'New verification code sent' : 'Failed to resend code'
    });
  } catch (error) {
...
// Registration route (updated with OTP verification)
app.post('/auth/register', [
  ...
  try {
    ...
    // Check if email was already registered while OTP was valid
...
app.post('/plans/select', isAuthenticated, async (req, res) => {
  try {
    const { plan_id } = req.body;
    ...
    await db.query(
      'UPDATE users SET plan_id = ?, product_limit = ? WHERE id = ?',
      [plan_id, plans[0].max_products, req.session.user.id]
    );

    // Update session
    req.session.user.plan_id = plan_id;
    req.session.user.product_limit = plans[0].max_products;
    ...
    req.session.message = {
      type: 'danger',
      text: 'Error selecting plan'
    };
    res.redirect('/plans/select');
  }
});
